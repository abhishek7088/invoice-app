...
const Terms = () => {
  ...
  useEffect(() => {
    const loadTexts = async () => {
      try {
        const data = await textService.getPageTexts('terms', language);
        setTexts(data.data);
      } catch (err) {
        console.error('Failed to load texts:', err);
        // Fallback content
        setTexts({
          terms_title: language === 'en' ? 'Terms' : 'Villkor',
          terms_button: language === 'en' ? 'Close and Go Back' : 'Stäng och gå tillbaka',
          terms_content: `BY clicking Invoice Now, you choose to register according to the information that you have typed in and the text on the registration page and the terms here, and you at the same time accept the terms here.
    ...
    <div className="terms-page">
      ...
      <div className="terms-header">
        ...
        <nav className="terms-nav">
          <Link to="/">{language === 'en' ? 'Home' : 'Hem'}</Link>
          <a href="#">{language === 'en' ? 'Order' : 'Beställ'}</a>
          <a href="#">{language === 'en' ? 'Our Customers' : 'Våra Kunder'}</a>
          <a href="#">{language === 'en' ? 'About us' : 'Om oss'}</a>
          <a href="#">{language === 'en' ? 'Contact Us' : 'Kontakta oss'}</a>
          
          <div className="language-switcher-nav" onClick={toggleLanguage}>
            <span>{language === 'en' ? 'English' : 'Svenska'}</span>
            <img 
              src={IMAGES.flags[language]} 
              alt={language === 'en' ? 'English' : 'Swedish'} 
              className="flag-icon-nav"
            />
          </div>
        </nav>
      </div>
      ...
      <div className="terms-main">
        <h1 className="terms-title">{texts.terms_title || 'Terms'}</h1>

        <button className="terms-close-btn" onClick={handleGoBack}>
          {texts.terms_button || 'Close and Go Back'}
        </button>

        <div className="terms-content-box">
          <div className="terms-text">
            {(texts.terms_content || '').split('\n\n').map((paragraph, index) => (
              <p key={index}>{paragraph}</p>
            ))}
          </div>
        </div>

        <button className="terms-close-btn-bottom" onClick={handleGoBack}>
          {texts.terms_button || 'Close and Go back'}
        </button>
      </div>
    </div>
